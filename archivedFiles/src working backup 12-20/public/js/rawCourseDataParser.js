export function rawCourseDataToJsonArray(data) {
  ...
  courseArray.push(
    ...
    JSON.parse(
      '{"id":"level100","courseTitle":["level100 Info"],"parentIds": [],"parentIdsComplex":[],"specificPrereq":"","trimester":["0"]}'
    )
  );
    ...
    while (prereqRegex.test(line.value)) {
      ...
      if (prereqLine.length == 1) {
        ...
        if (complexPrereqRegex.test(prereqLine)) {
          ...
          const extractedCourseArray = extractComplexPrereq(
            rawPrereqTokensArray
          );
